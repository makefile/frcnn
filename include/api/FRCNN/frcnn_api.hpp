...
using caffe::Frcnn::Point5f;
using caffe::Frcnn::RBBox;
...
public:
  ...
  void predict(const cv::Mat &img_in, vector<RBBox<float> > &results);
  void predict(const cv::Mat &img_in, vector<BBox<float> > &results);
  void predict_original(const cv::Mat &img_in, vector<RBBox<float> > &results);
