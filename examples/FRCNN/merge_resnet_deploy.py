...
def load_and_fill_biases(src_model, src_weights, dst_model, dst_weights, only_merge_freezed, is_train_proto=True):
    ...
    for i, layer in enumerate(model.layer):
        if layer.type == 'Convolution': # or layer.type == 'Scale':# Add bias layer if needed
            if only_merge_freezed and (layer.name[:4] in ['res3','res4','res5']): #layer.param[0].lr_mult == 1:  # 没有被freezed,训练时不能进行合并
                continue
            #print(i,layer.name,' === conv-bn-scale -> conv'))
...
def merge_batchnorms_in_net(net, pb_model, is_train):
    ...
    for i, layer in enumerate(net.layers):
        ...
        if l_name[:5] in ['conv1','res2a','res2b','res2c']:
            conv_freezed = True
            print(l_name, ' ==== conv-bn-scale -> conv')
        else:
            print(l_name, ' ==== conv-bn-scale -> conv-scale')
...
def main(args):
    if args.deploy is None:
        ...
        sys.exit(0)
    if args.phase=='train':
        merge_pb_weights(args,train_stage=True,model=args.deploy,is_train_proto=True)
    elif args.phase=='test':
        merge_pb_weights(args,train_stage=False,model=args.deploy)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--deploy', help="The net definition prototxt")
