...
//judge if a point is in the area of a rectangle
...
// get the four vertex's (x,y) coord of a rotated bbox
...
void _overlaps(float* overlaps,const float* boxes,const float* query_boxes, int n, int k, int device_id) {
    ...
    //fyk modify to pre alloc memory
    static float* overlaps_dev = NULL;
    static float* boxes_dev = NULL;
    static float* query_boxes_dev = NULL;
    static int capacity_n = 200000; //anchor nums
    static int capacity_k = 2000;   //proposal rois num
    if (overlaps_dev == NULL || n > capacity_n || k > capacity_k) {
        if (overlaps_dev != NULL){
            while (n > capacity_n) {
                capacity_n *= 2;
            }
            while (k > capacity_k) {
        ...
        CUDA_CHECK(cudaMalloc(&boxes_dev,
                    capacity_n * 5 * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&query_boxes_dev,
                    capacity_k * 5 * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&overlaps_dev,
                    capacity_n * capacity_k * sizeof(float)));
    }
...
// notice that we expect the rotate bbox's height and width is greater than 1, or the result is wrong.
template <typename Dtype>
std::vector<std::vector<Dtype> > get_rbbox_ious_gpu(const std::vector<float> &boxes,const std::vector<float> &query_boxes, Dtype type, int device_id) {
    // number of boxes
    int n = boxes.size() / 5;//(cx,cy,w,h,radian)
    int k = query_boxes.size() / 5;
    std::vector<std::vector<Dtype> > ious(n,std::vector<Dtype>(k));//array of [n*k]
    float *overlaps = new float[n * k];
    _overlaps(overlaps, &boxes[0], &query_boxes[0],n,k,device_id);
    for(int i=0;i<n;i++)
        for(int j=0;j<k;j++)
            ious[i][j] = overlaps[i * k + j];
    delete overlaps;
    //std::cout << "IOU n,k: " << n << " " << k << std::endl;
    return ious;
}
template std::vector<std::vector<float> > get_rbbox_ious_gpu(const std::vector<float> &boxes,const std::vector<float> &query_boxes, float type, int device_id);
template std::vector<std::vector<double> > get_rbbox_ious_gpu(const std::vector<float> &boxes,const std::vector<float> &query_boxes, double type, int device_id);
