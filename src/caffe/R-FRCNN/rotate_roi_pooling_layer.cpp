...
#include "rotate_roi_pooling_layer.hpp"
...
void RotateROIPoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  // For each ROI R = [batch_index Cx Cy width height angle]: max pool over R 
  for (int n = 0; n < num_rois; ++n) {
    ...
    Dtype w = bottom_rois[3];
    Dtype h = bottom_rois[4];
    Dtype angle = bottom_rois[5];// rad angle /180.0*3.1415926535;
