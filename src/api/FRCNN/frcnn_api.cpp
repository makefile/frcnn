...
void Detector::predict(const cv::Mat &img_in, std::vector<caffe::Frcnn::RBBox<float> > &results) {
  ...
  if (FrcnnParam::iter_test == -1) {
    predict_original(img_in, results);
      ...
      NOT_IMPLEMENTED;
      //similary as predict_original,but I donnot have time modify it right now,and I donnot use it.
    //predict_iterative(img_in, results);
  }
}

void Detector::predict_original(const cv::Mat &img_in, std::vector<caffe::Frcnn::RBBox<float> > &results) {
  ...
  for (int cls = 1; cls < cls_num; cls++) { 
    ...
    for (int i = 0; i < box_num; i++) { 
      ...
      Point5f<float> roi(rois->cpu_data()[(i * 6) + 1]/scale_factor,
                     ...
                     rois->cpu_data()[(i * 6) + 3]/scale_factor,
                     rois->cpu_data()[(i * 6) + 4]/scale_factor,
                     rois->cpu_data()[(i * 6) + 5]/scale_factor);

      Point5f<float> delta(bbox_pred->cpu_data()[(i * cls_num + cls) * 5 + 0],
                     bbox_pred->cpu_data()[(i * cls_num + cls) * 5 + 1],
                     bbox_pred->cpu_data()[(i * cls_num + cls) * 5 + 2],
                     bbox_pred->cpu_data()[(i * cls_num + cls) * 5 + 3],
                     bbox_pred->cpu_data()[(i * cls_num + cls) * 5 + 4]);

      Point5f<float> box = caffe::Frcnn::bbox_transform_inv(roi, delta);
      // boundary
      box[0] = std::max(0.0f, box[0]);
      box[0] = std::min(width-1.f, box[0]);
      box[1] = std::max(0.0f, box[1]);
      box[1] = std::min(height-1.f, box[1]);
      box[2] = std::max(0.0f, box[2]);
      box[3] = std::max(0.0f, box[3]);
      box[2] = std::min(width-1.f, box[2]);
      box[3] = std::min(height-1.f, box[3]);
      //donnot care about angle box[4]
      ...
      if (select[i]) {
        ...
        for (int j = i + 1; j < box_num; j++) {
          if (select[j]) {
            ...
            // also consider small angle differ
            else if (fabs(bbox[i][4] - bbox[j][4]) < M_PI/12)
              select[j] = false;
          }
        }
