...
class FrcnnAnchorTargetLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  Point5f<Dtype> _sum;
  Point5f<Dtype> _squared_sum;
  int _counts;
  int _fg_sum;
  int _bg_sum;
  int _count;
// For Debug
  inline void Info_Stds_Means_AvePos(const vector<Point5f<Dtype> > &targets, const vector<int> &labels){
//#ifdef DEBUG
#if 1
    ...
    Point5f<Dtype> means, stds;
    for (int j = 0; j < 5; j++) if (this->_counts > 0 ) {
    ...
    LOG_EVERY_N(INFO, 1000) << "STDS   : " << stds[0] << ", " << stds[1] << ", " << stds[2] << ", " << stds[3] << stds[4];
    LOG_EVERY_N(INFO, 1000) << "MEANS  : " << means[0] << ", " << means[1] << ", " << means[2] << ", " << means[3] << means[4];
    this->_count++;
