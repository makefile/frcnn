...
void FrcnnProposalTargetLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom,
  ...
  // fyk changed to (0, cx, cy, w, h, radian)
  top[0]->Reshape(1, 6, 1, 1);
  // labels
  top[1]->Reshape(1, 1, 1, 1);
  // bbox_targets
  top[2]->Reshape(1, config_n_classes_ * 5, 1, 1);
  // bbox_inside_weights
  top[3]->Reshape(1, config_n_classes_ * 5, 1, 1);
  // bbox_outside_weights
  top[4]->Reshape(1, config_n_classes_ * 5, 1, 1);
}
...
void FrcnnProposalTargetLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < bottom[1]->num(); i++) {
    ...
    gt_labels.push_back(bottom[1]->data_at(i,5,0,0));
  ...
  //targets
  vector<int> labels;
  vector<Point5f<Dtype> > rois;
  vector<vector<Point5f<Dtype> > > bbox_targets, bbox_inside_weights;
  ...
  DLOG(ERROR) << "top[0]-> " << batch_size << " , 6, 1, 1";
  // sampled rois
  top[0]->Reshape(batch_size, 6, 1, 1); // rois
  ...
  for (int i = 0; i < batch_size; i++) {
    ...
    rois_data[ top[0]->offset(i,5,0,0) ] = rois[i][4];
  }
  ...
  // bbox_targets
  top[2]->Reshape(batch_size, this->config_n_classes_*5, 1, 1);
  ...
  top[3]->Reshape(batch_size, this->config_n_classes_*5, 1, 1); //bbox_inside_weights
  caffe_set(top[3]->count(), Dtype(0), top[3]->mutable_cpu_data());
  Dtype *bbox_inside_data = top[3]->mutable_cpu_data();
  top[4]->Reshape(batch_size, this->config_n_classes_*5, 1, 1); //bbox_outside_weights
  ...
  for (int i = 0; i < batch_size; i++) {
    for (int j = 0; j < this->config_n_classes_; j++) {
      for (int cor = 0; cor < 5; cor++ ) {
        target_data[ top[2]->offset(i, j*5+cor, 0, 0) ] = bbox_targets[i][j][cor];
        bbox_inside_data[ top[2]->offset(i, j*5+cor, 0, 0) ] = bbox_inside_weights[i][j][cor];
        bbox_outside_data[ top[2]->offset(i, j*5+cor, 0, 0) ] = bbox_inside_weights[i][j][cor] > 0;
      }
    }
  }
...
template <typename Dtype>
void FrcnnProposalTargetLayer<Dtype>::_sample_rois(const vector<Point5f<Dtype> > &all_rois, const vector<Point5f<Dtype> > &gt_boxes, 
        const vector<int> &gt_label, const int fg_rois_per_image, const int rois_per_image, vector<int> &labels, 
        vector<Point5f<Dtype> > &rois, vector<vector<Point5f<Dtype> > > &bbox_targets, vector<vector<Point5f<Dtype> > > &bbox_inside_weights) {
// Generate a random sample of RoIs comprising foreground and background examples.

  CHECK_EQ(gt_label.size(), gt_boxes.size());
  // overlaps: (rois x gt_boxes)
  std::vector<std::vector<Dtype> > overlaps = skew_ious(all_rois, gt_boxes); 
  ...
  std::vector<Point5f<Dtype> > _gt_boxes(keep_inds.size());
  for (size_t i = 0; i < keep_inds.size(); ++ i) {
    labels[i] = _labels[keep_inds[i]];
    rois[i] = all_rois[keep_inds[i]];
    _gt_boxes[i] =
        gt_assignment[keep_inds[i]] >= 0 ? gt_boxes[gt_assignment[keep_inds[i]]] : Point5f<Dtype>();
  ...
  std::vector<Point5f<Dtype> > bbox_targets_data = bbox_transform(rois, _gt_boxes);
  ...
  bbox_targets = std::vector<std::vector<Point5f<Dtype> > >(
          keep_inds.size(), std::vector<Point5f<Dtype> >(this->config_n_classes_));
  bbox_inside_weights = std::vector<std::vector<Point5f<Dtype> > >(
          keep_inds.size(), std::vector<Point5f<Dtype> >(this->config_n_classes_));
  for (size_t i = 0; i < labels.size(); ++i) if (labels[i] > 0) {
    ...
    bbox_inside_weights[i][cls] = Point5f<Dtype>(FrcnnParam::bbox_inside_weights);
  }
