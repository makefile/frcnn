...
void FrcnnRoiDataLayer<Dtype>::DataLayerSetUp(
  ...
  while( data_load.load_WithDiff(infile) ) {
    ...
    // fyk change to include difficulty gound truth rois
    vector<vector<float> > rois = data_load.GetRois( true );
  ...
  //fyk changed to label cx,cy,w,h,theta
  //top[2]->Reshape(batch_size, 5, 1, 1);
  top[2]->Reshape(batch_size, 6, 1, 1);

  for (int i = 0; i < this->prefetch_.size(); ++i) {
    this->prefetch_[i]->label_.Reshape(batch_size + 1, 6, 1, 1);
  }
...
//fyk I put some check code in DataPrepare and disable width and height check and reset,so be make sure the input label is correct.
template <typename Dtype>
void FrcnnRoiDataLayer<Dtype>::CheckResetRois(vector<vector<float> > &rois, const string image_path, const float cols, const float rows, const float im_scale) {
  for (int i = 0; i < rois.size(); i++) {
    bool ok = rois[i][DataPrepare::CX] < cols && rois[i][DataPrepare::CY] < rows ;
    if (ok == false) {
      DLOG(INFO) << "Roi Data Check Failed : " << image_path << " [" << i << "]";
      DLOG(INFO) << " row : " << rows << ",  col : " << cols << ", im_scale : " << im_scale << " | " << rois[i][DataPrepare::CX] << ", " << rois[i][DataPrepare::CY] << ", " << rois[i][DataPrepare::W] << ", " << rois[i][DataPrepare::H] << ", " << rois[i][DataPrepare::THETA];
      rois[i][DataPrepare::CX] = std::min(cols-1, rois[i][DataPrepare::CX]);
      rois[i][DataPrepare::CY] = std::min(rows-1, rois[i][DataPrepare::CY]);
    }
  }
}
...
void FrcnnRoiDataLayer<Dtype>::FlipRois(vector<vector<float> > &rois, const float cols) {
  for (int i = 0; i < rois.size(); i++) {
    ...
    float old_cx = rois[i][DataPrepare::CX];
    rois[i][DataPrepare::CX] = cols - old_cx - 1;
    if(rois[i][DataPrepare::THETA] < M_PI_2)
    	rois[i][DataPrepare::THETA] = - rois[i][DataPrepare::THETA];//all between (-π/2,π/2]
  }
}
...
void FrcnnRoiDataLayer<Dtype>::load_batch(Batch<Dtype> *batch) {
  ...
  // fyk: modify here to adapt to angle.
  // label format:
  // labels x1 y1 x2 y2
  ...
  batch->label_.Reshape(channels, 6, 1, 1);
  ...
  //std::cout << "data layer rois num = " << rois.size() << std::endl;
  LOG_IF(INFO, rois.size()==0) << "data layer rois num = 0 , image: " << image_database_[index];
  for (int i = 1; i < channels; i++) {
    CHECK_EQ(rois[i-1].size(), DataPrepare::NUM);
    top_label[6 * i + 0] = rois[i-1][DataPrepare::CX] * im_scale; // cx
    top_label[6 * i + 1] = rois[i-1][DataPrepare::CY] * im_scale; // cy
    top_label[6 * i + 2] = rois[i-1][DataPrepare::W] * im_scale; // w
    top_label[6 * i + 3] = rois[i-1][DataPrepare::H] * im_scale; // h
    top_label[6 * i + 4] = rois[i-1][DataPrepare::THETA]; 	// theta
    top_label[6 * i + 5] = rois[i-1][DataPrepare::LABEL];         // label
//fyk maybe no need such precise bound.
/*
...
void FrcnnRoiDataLayer<Dtype>::Forward_cpu(
  ...
  if (this->output_labels_) {
    ...
    // Copy the labels.
    caffe_copy(batch->label_.count() - 6, batch->label_.cpu_data() + 6, top[2]->mutable_cpu_data());
  }
