...
detection* get_detections(vector<Blob<float>*> blobs,int img_w,int img_h, int netw, int neth, int *nboxes, int classes, float thresh, float nms)
{
    //int classes = 80;
    //float thresh = 0.5;
    ...
    for(int i=0;i<blobs.size();++i){
        ...
        forward_yolo_layer_cpu(blobs[i]->cpu_data(),l_params);
    }
