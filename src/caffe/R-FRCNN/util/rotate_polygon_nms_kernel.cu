...
#define box_dim 5 // (cx,cy,w,h,a) not include confidence,since the boxes has already sorted by confidence
__global__ void rotate_nms_kernel(const int n_boxes, const float nms_overlap_thresh,
  ...
  if (threadIdx.x < col_size) {
    for (int i = 0; i < box_dim; i ++) {  
      block_boxes[threadIdx.x * box_dim + i] =
        dev_boxes[(threadsPerBlock * col_start + threadIdx.x) * box_dim + i];
    }
  }
  __syncthreads();
  ...
  if (threadIdx.x < row_size) {
    ...
    for (i = start; i < col_size; i++) {
      if (devRotateIoU(cur_box, block_boxes + i * box_dim) > nms_overlap_thresh) {
        t |= 1ULL << i;
      }
    }
...
void _set_device_(int device_id) {
  int current_device;
...
// boxes_host not include confidence
void _rotate_nms(int* keep_out, int* num_out, const float* boxes_host, int boxes_num,
          int boxes_dim, float nms_overlap_thresh, int device_id) {
  _set_device_(device_id);

  float* boxes_dev = NULL;
  unsigned long long* mask_dev = NULL;
  const int col_blocks = DIVUP(boxes_num, threadsPerBlock);

  // fyk: change the manner of frequently malloc and free to malloc less and ignore free, I do this because I need run multi gpu program in 1 machine,they would compete for memory
  static int capacity = 6000; // can also set a MAX value defined in config: rpn_pre_nms
  static float* g_boxes_dev = NULL;
  static unsigned long long* g_mask_dev = NULL;
  
  if (g_boxes_dev == NULL || boxes_num > capacity) {
      //malloc or realloc
      if (g_boxes_dev != NULL) {
        CUDA_CHECK(cudaFree(g_boxes_dev));
      }
      while (boxes_num > capacity) {
          capacity *= 2;//same as std::vector's capacity strategy
      }
      CUDA_CHECK(cudaMalloc(&g_boxes_dev,
                            capacity * boxes_dim * sizeof(float)));
      CUDA_CHECK(cudaMalloc(&g_mask_dev,
                            capacity * col_blocks * sizeof(unsigned long long)));      
  }
  int offset = capacity - boxes_num;
  int box_oft = offset * (boxes_dim * sizeof(float));
  int mask_oft = offset * (col_blocks * sizeof(unsigned long long));
  boxes_dev = g_boxes_dev + box_oft;
  mask_dev  = g_mask_dev + mask_oft;
  ...
  *num_out = num_to_keep;
  ...
  // fyk: do not care about memory free,leave this for OS to release.
